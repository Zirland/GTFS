...
if ($result69 = mysqli_query($link, $akt_route)) {
    while ($row69 = mysqli_fetch_row($result69)) {
		...
		$current = "N$route_id,$agency_id,\"$route_short_name\",\"$route_long_name\",$route_type,$route_color,$route_text_color\n";
...
$prevnow = $now;
				...
				if ($dec != 1)  {
					...
					$mark_cal = mysqli_query($link, "INSERT INTO kango.cal_use (trip_id, kalendar) VALUES ('N$trip_id', '$service_id');");
// zápis kalendáře spoje pro tento týden do databáze
					...
					$current = "N$route_id,$service_id,N$trip_id,\"$trip_headsign\",$direction_id,N$shape_id,$wheelchair_accessible,$bikes_allowed\n";
					$file = 'trips.txt';
					file_put_contents($file, $current, FILE_APPEND);
					$tripnums = $tripnums + 1;
// zapsán aktivní spoj

					$query171 = "INSERT INTO shapecheck (trip_id, shape_id) VALUES ('N$trip_id', '$shape_id');";
...
$prevnow = $now;
					...
					if ($result166 = mysqli_query($link, $tripstops)) {
						while ($row166 = mysqli_fetch_row($result166))  {
							...
							$current = "N$trip_id,$arrival_time,$departure_time,$stop_id,$stop_sequence,$pickup_type,$drop_off_type\n";
							$file = 'stop_times.txt';
							file_put_contents($file, $current, FILE_APPEND);
			
							$mark_stop = mysqli_query($link, "INSERT INTO kango.stop_use (trip_id, stop_id) VALUES ('N$trip_id', '$stop_id');");
